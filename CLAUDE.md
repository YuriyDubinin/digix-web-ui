# CLAUDE.md — Бриф для агента: фронтенд сайта Digix

Этот файл — самодостаточный концентрат для агента, который будет создавать фронтенд
сайта компании **Digix** в отдельном проекте. Бэкенд (Go REST API) уже существует и
...
**Digix** — компания, занимающаяся **инновационными цифровыми решениями для среднего
...
Проект живёт в **отдельной папке/репозитории** (название — `digix-web`). Структура:
...
2. **Hero** (100vh)
   ...
   - Подзаголовок: 1–2 предложения о Digix
...
```
docker build --build-arg VITE_API_BASE_URL=http://localhost:18080 -t digix-web .
...
- [ ] `docker build -t digix-web .` собирается; `docker run -p 3000:80 digix-web` поднимает сайт на 3000.
...
- Бэкенд этого проекта: `https://github.com/YuriyDubinin/digix-api`
...
**Целевая аудитория:** AI-агент или разработчик, начинающий фронтенд Digix с нуля.
